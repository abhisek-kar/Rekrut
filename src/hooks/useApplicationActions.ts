...
export function useApplicationActions({ userRole, onSuccess }: ApplicationActionsProps) {
  ...
  const handleApplicationAction = useCallback((actionType: string, applicationIds: string | string[]) => {
    const applicationArray = Array.isArray(applicationIds) ? applicationIds : [applicationIds];
    
    // Edge case: Prevent subadmins from using bulk assign action
    if (actionType === 'assign' && applicationArray.length > 1 && userRole === 'subadmin') {
      toast.error('Bulk assignment is not available for SubAdmins. Please assign applications individually.');
      return;
    }
  ...
  const executeAction = useCallback(async () => {
    ...
    try {
      ...
      if (isBulkAction) {
        // Use bulk API
        const apiEndpoint = userRole === 'admin' ? '/api/applications/bulk' : '/api/subadmin/applications/bulk';
        ...
        toast.success(result.message || `${targetApplications.length} applications updated successfully`);
      } else {
        // Use individual application API
        const applicationId = targetApplications[0];
        await executeSingleApplicationAction(applicationId, currentAction, actionData);
      }
  ...
  const executeSingleApplicationAction = async (applicationId: string, action: string, data: any) => {
    switch (action) {
      case 'status':
      ...
      case 'assign':
        ...
        break;
      case 'note':
        await addApplicationNote(applicationId, data.note, data.type, data.isInternal);
        break;
      case 'archive':
  ...
  const assignApplication = async (applicationId: string, assignedTo: string) => {
    const apiEndpoint = userRole === 'admin' 
  ...
  const addApplicationNote = async (applicationId: string, note: string, type: string, isInternal: boolean) => {
    const apiEndpoint = userRole === 'admin' 
  ...
  const downloadApplication = useCallback(async (applicationId: string, format: 'pdf' | 'csv' = 'pdf') => {
    try {
      const apiEndpoint = userRole === 'admin' 
  ...
  // Email application
  const emailApplication = useCallback(async (applicationId: string, emailType: 'candidate' | 'internal') => {
    try {
      const apiEndpoint = userRole === 'admin' 
