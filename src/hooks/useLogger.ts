...
export function useLogger() {
  ...
  const logUserAction = useCallback((action: string, details?: Record<string, unknown>) => {
    info(`User action: ${action}`, { action, ...details } as Record<string, unknown>);
  }, [info]);

  const logError = useCallback((component: string, err: Error, context?: Record<string, unknown>) => {
    error(`Component error in ${component}`, err, { component, ...context } as Record<string, unknown>);
  }, [error]);

  const logPerformance = useCallback((operation: string, duration: number, context?: Record<string, unknown>) => {
    const level = duration > 1000 ? 'warn' : 'debug';
    const logger = level === 'warn' ? warn : debug;
    logger(`Performance: ${operation} took ${duration}ms`, { operation, duration, ...context } as Record<string, unknown>);
  }, [warn, debug]);

  const logNavigation = useCallback((from: string, to: string, context?: Record<string, unknown>) => {
    debug(`Navigation: ${from} → ${to}`, { from, to, ...context } as Record<string, unknown>);
  }, [debug]);
...
export function usePerformanceLogger() {
  ...
  const measureAsync = useCallback(async <T>(
    ...
    try {
      ...
      const duration = performance.now() - startTime;
      logPerformance(`${operation} (failed)`, duration, { ...(context || {}), error } as Record<string, unknown>);
      throw error;
    }
  }, [logPerformance]);
