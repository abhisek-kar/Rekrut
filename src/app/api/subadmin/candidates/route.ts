...
export async function GET(request: NextRequest) {
  try {
    ...
    const candidatesWithApplications = await Promise.all(
      candidates.map(async (candidate) => {
        ...
        if (applicationStatus && applicationStatus !== 'all') {
          filteredApplications = candidateApplications.filter(app => app.status === applicationStatus);
        }
        ...
        return {
          ...
          latestApplicationDate: latestApplication?.applicationDate,
    ...
    if (applicationStatus && applicationStatus !== 'all') {
      filteredCandidates = candidatesWithApplications.filter(candidate => 
    ...
    const stats = {
      ...
      inactive: allCandidatesInJobs.filter(c => c.status === 'inactive').length,
      blacklisted: allCandidatesInJobs.filter(c => c.status === 'blacklisted').length,
      byApplicationStatus: {
        applied: 0,
        screening: 0,
        interview_scheduled: 0,
    ...
    applicationStatusCounts.forEach(({ _id, count }) => {
      if (stats.byApplicationStatus.hasOwnProperty(_id)) {
        stats.byApplicationStatus[_id as keyof typeof stats.byApplicationStatus] = count;
      }
    });
