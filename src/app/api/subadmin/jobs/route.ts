...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get jobs with pagination (keeping _id for internal aggregation)
    const jobsWithId = await Job.find(query)
    ...
    const jobsWithApplicationCounts = await Promise.all(
      jobsWithId.map(async (job) => {
        ...
        const { _id: jobId, ...jobWithoutId } = job;
        return {
          ...jobWithoutId,
