 ...
 */
export async function canAccessJob(jobId: string, session: Session | null): Promise<boolean> {
 ...
 */
export async function canAccessApplication(applicationId: string, session: Session | null): Promise<boolean> {
  if (!session?.user) return false;
 ...
 */
export async function canModifyJob(jobId: string, session: Session | null): Promise<boolean> {
  if (!session?.user) return false;
 ...
 */
export async function canAssignJob(jobId: string, session: Session | null): Promise<boolean> {
  if (!session?.user) return false;
 ...
 */
export async function canModifyApplication(applicationId: string, session: Session | null): Promise<boolean> {
  if (!session?.user) return false;
  ...
  try {
    ...
    console.error('Error checking application modification permissions:', error);
    return false;
  }
}
