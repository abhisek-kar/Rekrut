...
function replaceVariables(content: string, variables: EmailTemplateVariables): string {
...
function createButton(text: string, url: string, color: string = "#4f46e5"): string {
  return `
...
export function generateJobAssignmentEmail(variables: {
  ...
  const content = `
    ...
    <div style="background-color: #f1f5f9; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #4f46e5;">
      <h3 style="color: #1e293b; margin: 0 0 10px 0; font-size: 18px;">${variables.jobTitle}</h3>
...
export function generateJobApplicationReceivedEmail(variables: {
  ...
  const content = `
    ...
    <div style="background-color: #f1f5f9; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #10b981;">
      <h3 style="color: #1e293b; margin: 0 0 15px 0; font-size: 18px;">${variables.jobTitle}</h3>
      <div style="color: #64748b; font-size: 14px; line-height: 1.6;">
        <p style="margin: 0 0 8px 0;"><strong>Candidate:</strong> ${variables.candidateName}</p>
        <p style="margin: 0 0 8px 0;"><strong>Email:</strong> ${variables.candidateEmail}</p>
        <p style="margin: 0;"><strong>Applied on:</strong> ${variables.applicationDate}</p>
      </div>
    </div>
...
export function generateJobStatusChangeEmail(variables: {
    ...
    <div style="background-color: #f1f5f9; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid ${statusColor};">
      ...
      <div style="color: #64748b; font-size: 14px; line-height: 1.6;">
        <p style="margin: 0 0 8px 0;">
          ...
          <span style="text-transform: capitalize;">${variables.oldStatus}</span>
        </p>
        <p style="margin: 0 0 8px 0;">
          <strong>Status changed to:</strong> 
          <span style="color: ${statusColor}; font-weight: bold; text-transform: capitalize;">${variables.newStatus}</span>
        </p>
        <p style="margin: 0 0 8px 0;"><strong>Changed by:</strong> ${variables.changedBy}</p>
        ${variables.reason ? `<p style="margin: 0;"><strong>Reason:</strong> ${variables.reason}</p>` : ''}
      </div>
    </div>
  ...
  return {
    ...
    text: `Hello ${variables.recipientName},\n\nThe status of "${variables.jobTitle}" has been changed from "${variables.oldStatus}" to "${variables.newStatus}" by ${variables.changedBy}.\n\n${variables.reason ? `Reason: ${variables.reason}\n\n` : ''}View job: ${variables.jobUrl}\n\nBest regards,\nThe Rekrut Team`
  };
}
...
export function generateApplicationConfirmationEmail(variables: {
  ...
  const content = `
    ...
    <div style="background-color: #f0f9ff; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #0ea5e9;">
      <h3 style="color: #1e293b; margin: 0 0 15px 0; font-size: 18px;">${variables.jobTitle}</h3>
      <div style="color: #64748b; font-size: 14px; line-height: 1.6;">
        <p style="margin: 0 0 8px 0;"><strong>Company:</strong> ${variables.companyName}</p>
        <p style="margin: 0 0 8px 0;"><strong>Applied on:</strong> ${variables.applicationDate}</p>
        ${variables.trackingToken ? `<p style="margin: 0;"><strong>Application ID:</strong> ${variables.trackingToken}</p>` : ''}
      </div>
    </div>
    ...
    ${variables.statusUrl ? createButton("Track Application Status", variables.statusUrl, "#0ea5e9") : ''}
  ...
  return {
    ...
    text: `Dear ${variables.candidateName},\n\nThank you for applying for ${variables.jobTitle} at ${variables.companyName}.\n\nWe have successfully received your application on ${variables.applicationDate}.\n\nOur recruitment team will review your application and contact you within 5-7 business days if your profile matches our requirements.\n\n${variables.trackingToken ? `Application ID: ${variables.trackingToken}\n` : ''}${variables.statusUrl ? `Track your application: ${variables.statusUrl}\n` : ''}\nBest regards,\nThe ${variables.companyName} Team`
  };
}
...
export function generateApplicationStatusChangeEmail(variables: {
  ...
  const statusMessages: Record<string, { title: string; color: string; icon: string }> = {
    screened: { title: "Application Under Review", color: "#0ea5e9", icon: "👀" },
    interview_scheduled: { title: "Interview Scheduled", color: "#10b981", icon: "📅" },
  ...
  const content = `
    ...
    <p style="color: #334155; font-size: 16px; line-height: 1.6;">
      We have an update regarding your application for <strong>${variables.jobTitle}</strong> at ${variables.companyName}.
    </p>
    
    <div style="background-color: #f1f5f9; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid ${statusInfo.color};">
      <h3 style="color: #1e293b; margin: 0 0 15px 0; font-size: 18px;">${variables.jobTitle}</h3>
      <div style="color: #64748b; font-size: 14px; line-height: 1.6;">
        <p style="margin: 0 0 8px 0;">
          <strong>Status:</strong> 
          <span style="color: ${statusInfo.color}; font-weight: bold; text-transform: capitalize;">
    ...
    ${variables.interviewDetails ? `
      <div style="background-color: #f0fdf4; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #10b981;">
        ...
        <div style="color: #166534; font-size: 14px; line-height: 1.6;">
          <p style="margin: 0 0 8px 0;"><strong>Date:</strong> ${variables.interviewDetails.date}</p>
          <p style="margin: 0 0 8px 0;"><strong>Time:</strong> ${variables.interviewDetails.time}</p>
          ${variables.interviewDetails.location ? `<p style="margin: 0 0 8px 0;"><strong>Location:</strong> ${variables.interviewDetails.location}</p>` : ''}
          ${variables.interviewDetails.interviewers && variables.interviewDetails.interviewers.length > 0 ? 
            `<p style="margin: 0;"><strong>Interviewers:</strong> ${variables.interviewDetails.interviewers.join(', ')}</p>` : ''}
        </div>
      </div>
    ` : ''}
  ...
  return {
    ...
    text: `Dear ${variables.candidateName},\n\nWe have an update regarding your application for ${variables.jobTitle} at ${variables.companyName}.\n\nStatus: ${variables.newStatus.replace('_', ' ')}\n\n${variables.message ? `${variables.message}\n\n` : ''}${variables.nextSteps ? `Next Steps: ${variables.nextSteps}\n\n` : ''}Best regards,\nThe ${variables.companyName} Team`
  };
}
...
export function generateAccountCreatedEmail(variables: {
  ...
  const content = `
    ...
    <div style="background-color: #f1f5f9; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #4f46e5;">
      ...
      <div style="color: #64748b; font-size: 14px; line-height: 1.6;">
        <p style="margin: 0 0 8px 0;"><strong>Email:</strong> ${variables.email}</p>
        <p style="margin: 0;"><strong>Role:</strong> ${variables.role.charAt(0).toUpperCase() + variables.role.slice(1)}</p>
      </div>
    </div>
...
export function generateBulkOperationCompleteEmail(variables: {
  ...
  const content = `
    ...
    <div style="background-color: #f0fdf4; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #10b981;">
      ...
      <div style="color: #166534; font-size: 14px; line-height: 1.6;">
        <p style="margin: 0 0 8px 0;"><strong>Operation:</strong> ${variables.operationType}</p>
        <p style="margin: 0 0 8px 0;"><strong>Items affected:</strong> ${variables.affectedCount} ${variables.entityType}(s)</p>
  ...
  return {
    ...
    text: `Hello ${variables.recipientName},\n\nYour bulk operation has been completed successfully.\n\nOperation: ${variables.operationType}\nItems affected: ${variables.affectedCount} ${variables.entityType}(s)\nCompleted at: ${new Date().toLocaleString()}\n\n${variables.details ? `Details: ${variables.details}\n\n` : ''}Best regards,\nThe Rekrut Team`
  };
}
