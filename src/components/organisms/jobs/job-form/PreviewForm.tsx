...
export function PreviewForm({ data, onChange, onValidityChange }: PreviewFormProps) {
  // Initialize the form
            ...
            <Card>
              <CardHeader>
                <div className="flex justify-between items-start">
                  <div>
                    <CardTitle className="text-2xl">{data.title || 'Job Title'}</CardTitle>
                    <div className="flex items-center mt-2">
                      <Building className="h-4 w-4 mr-1 text-muted-foreground" />
                      <span className="text-muted-foreground">{data.company || 'Company'}</span>
                      {data.department && (
                        <>
                          <span className="mx-1 text-muted-foreground">•</span>
                          <span className="text-muted-foreground">{data.department}</span>
                        </>
                      )}
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    <Badge className={jobTypeColors[data.employmentType || 'full-time']}>
                ...
                <div className="flex flex-wrap gap-4 mt-2">
                  <div className="flex items-center text-sm">
                    ...
                    <span>
                      {data.location?.type
                        ? data.location.type.charAt(0).toUpperCase() + data.location.type.slice(1)
                        : 'Location'}
                      {data.location?.type !== 'remote' && data.location?.city && (
                        <> • {data.location.city}</>
                      )}
                    </span>
                  </div>
                  <div className="flex items-center text-sm">
                    ...
                    <span>
                      {data.experienceLevel
                        ? `${data.experienceLevel.charAt(0).toUpperCase() + data.experienceLevel.slice(1)} Level`
                        : 'Experience'}
                    </span>
                  </div>
                  {data.salary?.visible && (
                  ...
                  {data.applicationDeadline && (
                    <div className="flex items-center text-sm">
                      ...
                      <span>Apply by {formatDate(data.applicationDeadline)}</span>
                    </div>
                  )}
                </div>
              </CardHeader>
              ...
              <CardContent className="space-y-6">
                ...
                {data.educationRequirements && data.educationRequirements.length > 0 && (
                  <div className="space-y-2">
                ...
                {data.applicationInstructions && (
                  <div className="space-y-2">
                    <h3 className="text-lg font-medium">Application Instructions</h3>
              ...
              <CardFooter className="flex justify-between pt-6 border-t">
                ...
                <Button size="lg" className="bg-primary">Apply Now</Button>
              </CardFooter>
            </Card>
          </TabsContent>
              ...
              <CardContent className="space-y-6">
                ...
                {data.referralBonus && (
                  <div className="p-3 bg-amber-50 border border-amber-200 rounded-md flex items-center">
                    <span className="font-medium text-amber-800">Referral Bonus:</span>
                    <span className="ml-2 text-amber-800">{data.referralBonus}</span>
                  </div>
                )}
                  ...
                  <div className="space-y-4 border p-4 rounded-md">
                    ...
                    <div className="space-y-2">
                      <div>
                        ...
                        <p>{data.seoTitle || data.title || 'Default job title will be used'}</p>
                      </div>
                      <div>
                        <p className="text-sm font-medium text-muted-foreground">SEO Description</p>
                        <p className="text-sm">
                          {data.seoDescription || 'Default job description will be used for search engines'}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center p-3 bg-green-50 border border-green-200 rounded-md">
              <div className="space-y-1">
                ...
                <p className="text-sm text-green-700">Your job posting is ready to be published. Click the "Publish Job" button to make it active.</p>
              </div>
            </div>
          </CardContent>
        </Card>
