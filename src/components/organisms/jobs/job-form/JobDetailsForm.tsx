...
export function JobDetailsForm({ data, onChange, onValidityChange }: JobDetailsFormProps) {
  ...
  useEffect(() => {
    const subscription = form.watch(() => {
      ...
      const isValid = !!values.description && !!values.responsibilities && 
                    !!values.requirements && values.skills.length > 0;
  ...
  const handleRemoveSkill = (skillToRemove: string) => {
    const updatedSkills = form.getValues().skills.filter(skill => skill !== skillToRemove);
    form.setValue('skills', updatedSkills);
  };

  // Add a new education requirement
  const handleAddEducation = () => {
    if (newEducation.trim() && !form.getValues().educationRequirements?.includes(newEducation.trim())) {
      const updatedEducation = [...(form.getValues().educationRequirements || []), newEducation.trim()];
  ...
  const handleRemoveEducation = (educationToRemove: string) => {
    const updatedEducation = form.getValues().educationRequirements?.filter(
    ...
    <Form {...form}>
      <form className="space-y-6">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                Include a thorough overview of the role, its purpose within the organization, and key objectives
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Responsibilities <span className="text-destructive">*</span></FormLabel>
              <FormControl>
                <Textarea 
              ...
              <FormDescription>
                Detail the specific tasks, responsibilities, and expectations for this role
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Requirements <span className="text-destructive">*</span></FormLabel>
              <FormControl>
                <Textarea 
              ...
              <FormDescription>
                Specify the qualifications, experience, and attributes candidates should possess
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={() => (
            <FormItem>
              ...
              <FormDescription>
                List specific skills required for the role (technical, soft skills, etc.)
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={() => (
            <FormItem>
              ...
              <div className="space-y-4">
                ...
                <div>
                  {!form.getValues().educationRequirements?.length ? (
                    <div className="text-sm text-muted-foreground">No education requirements added yet</div>
                  ) : (
                    <div className="flex flex-wrap gap-2">
                      {form.getValues().educationRequirements.map((education, index) => (
                        <Badge 
                          key={index} 
                          variant="outline"
                          className="py-1.5 px-2 text-sm"
              ...
              <FormDescription>
                Specify educational qualifications desired for this position (optional)
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
          ...
          <CardContent className="p-4">
            <div className="flex gap-2">
              ...
              <div className="space-y-2">
                <h4 className="text-sm font-medium">Tips for writing effective job details:</h4>
                <ul className="text-sm text-muted-foreground list-disc pl-5 space-y-1">
                  <li>Be specific about day-to-day responsibilities</li>
                  <li>Distinguish between "required" and "preferred" qualifications</li>
