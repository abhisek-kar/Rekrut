...
const DocumentUploadStep: React.FC<DocumentUploadStepProps> = ({
  ...
  const isDocumentRequired = (documentType: string) => {
    ...
    return requiredDocs.some(
      (reqDoc) =>
        ...
        (docType === "resume" &&
          (reqDoc.includes("cv") || reqDoc.includes("resume"))) ||
    ...
    if (!allowedTypes.includes(file.type)) {
      const docTypeDisplay =
        type === "resume"
          ...
          ? "Cover Letter"
          : "Portfolio";
      const supportedFormats = getFileAcceptTypes(type)
        .split(",")
        .map((ext) => ext.replace(".", "").toUpperCase())
        .join(", ");
      alert(
        `Invalid file type for ${docTypeDisplay}. Please upload a file in one of these formats: ${supportedFormats}`
      );
      return;
    }
  ...
  const getRequiredDocuments = () => {
    ...
    if (
      requiredDocs.some((doc) => doc.includes("resume") || doc.includes("cv"))
    ) {
    ...
    if (
      requiredDocs.some(
        (doc) => doc.includes("portfolio") || doc.includes("work samples")
      )
    ) {
    ...
    <div className="w-full max-w-2xl mx-auto">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Upload your resume and any additional documents to support your
            application.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-6">
              ...
              <p className="text-sm text-muted-foreground">
                ...
                {!job?.requiredDocuments?.length && (
                  <span className="block mt-1">
                    <strong>Resume/CV is always required.</strong> Other
                    documents are optional unless specified.
                  </span>
                )}
              </p>
            </div>
            ...
            <FileUploadArea
              ...
              label="Cover Letter"
              description={`${
                isDocumentRequired("coverLetter") ? "Required" : "Optional"
            ...
            {/* <FileUploadArea
              ...
              description={`${
                isDocumentRequired("portfolio") ? "Required" : "Optional"
              } - Supported formats: PDF, ZIP, JPG, PNG (max 10MB)`}
